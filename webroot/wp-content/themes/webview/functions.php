...
function remove_menus () {
  ...
  if ( current_user_can( 'administrator' ) ) {
    switch( $_SERVER['REMOTE_ADDR']){
      case '127.0.0.1':     // lcoalhost
      case '192.168.0.1':     // lcoalhost
      case '::1':        // localhost
      case '192.168.14.1':  // vagrant
      case '210.172.0.33':  // xtone
      case '39.110.212.244':  // FQ(tokyo)
...
//----------------------------
//カスタム投稿タイプのみアーカイブでの表示件数
//----------------------------
add_action('pre_get_posts', 'my_pre_get_posts');
function my_pre_get_posts($query) {
    if (!is_admin() && $query->is_main_query() && is_post_type_archive(array('case','column','laboratory','special'))) {
